...
export class ProductPolyCarbonateRollCalculationDialogComponent {
  ...
  /**
   * Handles Tab key press on the "length" input field (first field in each row)
   * Automatically adds a new row when tabbing out of the last row's "length" field
   * @param event - Keyboard event
   * @param rowIndex - Current row index
   */
  onLengthTabKey(event: KeyboardEvent, rowIndex: number): void {
    ...
    if (event.key === 'Tab' && !event.shiftKey) {
      ...
      if (isLastRow) {
        ...
        // Focus on the first input field (length) of the new row after DOM update
        setTimeout(() => {
          const newRowIndex = this.calculationsArray.length - 1;
          const firstInputSelector = `input[formcontrolname="length"]`;
