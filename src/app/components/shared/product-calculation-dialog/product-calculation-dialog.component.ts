...
export class ProductCalculationDialogComponent {
  ...
  private calculateRow(index: number): void {
    ...
    if(this.calculationType === ProductCalculationType.SQ_FEET) {
      ...
      // Calculate sqFeet based on product type
      const sqFeet = this.product.type.toUpperCase() === ProductMainType.REGULAR.toUpperCase()
        ? this.productCalculationService.calculateSqFeet(calcRunningFeet, this.product)
        : calcRunningFeet * weightMultiplier;
    ...
    if (this.calculationForm.valid) {
      const latestCalculations = this.calculationsArray.controls.map(control => {
        ...
        return this.calculationType === 'MM' ? {
          ...
          sqFeet: this.product.type === 'POLY_CARBONATE' ? Number(values.sqFeet?.toFixed(3)) || 0 : Number(values.sqFeet?.toFixed(2)) || 0,
  ...
  // Add keyboard shortcut listener for Alt+R and Alt+S
  ...
  /**
   * Handles Tab key press on the "nos" input field
   * Automatically adds a new row when tabbing out of the last row's "nos" field
   * @param event - Keyboard event
   * @param rowIndex - Current row index
   */
  onNosTabKey(event: KeyboardEvent, rowIndex: number): void {
    // Check if Tab key is pressed (not Shift+Tab for backwards navigation)
    if (event.key === 'Tab' && !event.shiftKey) {
      // Check if this is the last row
      const isLastRow = rowIndex === this.calculationsArray.length - 1;
      ...
      if (isLastRow) {
        ...
        // Focus on the first input field of the new row after DOM update
        setTimeout(() => {
          const newRowIndex = this.calculationsArray.length - 1;
          const firstInputSelector = this.calculationType === 'MM' 
            ? `input[formcontrolname="mm"]` 
            : `input[formcontrolname="feet"]`;
          
          // Get all matching inputs and focus on the one for the new row
          const inputs = document.querySelectorAll(firstInputSelector);
          if (inputs && inputs[newRowIndex]) {
            (inputs[newRowIndex] as HTMLElement).focus();
          }
        }, 0);
      }
    }
